...
>[!WARNING]
>
>Sommige **Pro projecten** vereisen hulp van de Steun van Adobe om de routeconfiguraties in het `routes.yaml` dossier en de kroonconfiguraties in het `.magento.app.yaml` dossier bij te werken. Adobe adviseert het maken van en het bevestigen van alle configuratieveranderingen YAML in een milieu van de Integratie eerst, en dan het opstellen van hen aan het Opvoeren milieu.
>
>
>Als uw veranderingen niet op het Opvoeren van plaatsen na herplaatsing en geen verwante foutenmeldingen in het logboek worden weerspiegeld, moet u **[&#x200B; een kaartje van de Steun van Adobe Commerce voorleggen &#x200B;](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/help-center-guide/magento-help-center-user-guide.html?lang=nl-NL#submit-ticket).** In het kaartje, beschrijf duidelijk de configuratieveranderingen u probeerde en om het even welke bijgewerkte configuratiedossiers YAML in het kaartje vastmaakt.
...
- Geef aan welke servicewijzigingen nodig zijn, neem de bijgewerkte `.magento.app.yaml` - en `services.yaml` -bestanden op en geef de PHP-versie op in het ticket. Voor zelfbedienings veranderingen in PHP versie, uitbreidingen, of milieu montages, zie [&#x200B; PHP montages &#x200B;](https://experienceleague.adobe.com/docs/commerce-on-cloud/user-guide/configure/app/php-settings.html?lang=nl-NL) in _configuratie van de Toepassing_.
  ...
  >[!IMPORTANT]
  >
  >Wanneer het selecteren van het gebied van het Milieu in de nieuwe kaartvorm, gebruiks het milieu van Adobe noemen. Bijvoorbeeld, uitgezocht het Opvoeren zelfs als u dat milieu **Dev** intern roept. U kunt uw interne naam in de beschrijving vermelden, maar in het veld Milieu zelf moet de Adobe-nomenclatuur worden gebruikt.
...
- Voor veranderingen in een levende milieu van de Productie (**slechts Pro**), wordt een minimum van 48 uurverklaring vereist. Hierdoor kan het infrastructuurteam van de cloud voldoende tijd krijgen om bronnen te bundelen en een veilige upgrade uit te voeren. De opzegtermijn begint wanneer het infrastructuurteam de aanvraag erkent en de upgrade plant, met uitzondering van weekends. Bijvoorbeeld, om de dienstverbeteringen te hebben op een maandag worden voltooid, moet een erkenning van de geplande verbetering tegen Woensdag worden ontvangen. Tijdens piekvraagperiodes, zou het meer tijd kunnen vergen om uw verzoek te verwerken.
  ...
  >[!NOTE]
  >
  >Alle geplande onderhoudsperioden moeten in UTC-formaat worden verstrekt om te zorgen voor duidelijkheid en consistentie in alle communicatie. De verbeteringen van de dienst kunnen niet in het het Staging milieu worden gepland; in de meeste gevallen, worden de verbeteringen in het Staging uitgevoerd op de zelfde dag als het verzoek.
  >
  >Als u om een verbetering verzoekt RabbitMQ, zorg ervoor om het milieu opnieuw op te stellen nadat de verbetering voltooit zodat de berichtrijen worden re-geïnitialiseerd.

- **two-Part Handshake Process voor het plannen van Upgrades**

  Om een vlot en gecoördineerd verbeteringsproces te verzekeren, volgt de Steun van Adobe Commerce een twee-deel handshake proces voor alle verbeteringen van het productiemilieu:

   1. **Bevestiging van de Klant**: De Steun van Adobe verzoekt eerst dat de klant de gewenste datum en de tijd voor de verbetering bevestigt. Deze stap zorgt ervoor dat de timing zich op de bedrijfsbehoeften van de klant en de onderhoudsvensters richt.
   2. **Plannend &amp; Definitieve Bevestiging**: Zodra de klant de timing bevestigt, legt de Steun van Adobe het verzoek aan het team van de Infrastructuur voor, die dan het verzoek en de definitieve bevestiging van het geplande verbeteringsvenster overziet.

De upgrade wordt pas gepland nadat het infrastructuurteam de definitieve bevestiging heeft gegeven. Klanten worden aangespoord om ten minste 48 uur voor het upgradevenster snel te reageren om vertragingen te voorkomen en een passende kennisgeving mogelijk te maken.
