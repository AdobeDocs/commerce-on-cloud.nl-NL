...
- `false` - `Consumers` proces beschikbare berichten in de rij, sluit de verbinding van TCP, en beëindigt. `Consumers` Wacht niet op extra berichten om de rij in te gaan, zelfs als het aantal verwerkte berichten minder dan de `max_messages` waarde is die in `CRON_CONSUMERS_RUNNER` wordt gespecificeerd plaatsend variabele.
