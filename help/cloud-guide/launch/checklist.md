...
Als u dit certificaat wilt gebruiken, moet u de DNS-configuratie bijwerken, zodat Adobe de domeinvalidatie kan voltooien en het certificaat op uw omgeving kan toepassen. Elke omgeving heeft een uniek certificaat dat de domeinen voor de Adobe Commerce bestrijkt op cloudinframinframites die in die omgeving worden geïmplementeerd. Wij adviseren de voltooiing en de configuratieupdates tijdens het [ Snelle opstellingsproces ](../cdn/fastly-configuration.md).
...
- SSL/TLS-certificaat dat is ingericht voor uw productieomgeving.
  ...
  Als u de ACME uitdagingsverslagen voor uw domeinen van de Productie tijdens het Fastly opstellingsproces toevoegde, uploadt Adobe het SSL/TLS certificaat aan uw milieu van de Productie automatisch wanneer u de DNS configuratie aan routeverkeer aan de Fastly dienst bijwerkt. Als u het certificaat niet vooraf hebt geleverd of als u uw domeinen hebt bijgewerkt, moet Adobe de domeinvalidatie voltooien en het certificaat leveren. Dit kan 12 uur in beslag nemen.
...
1. Indien nodig, voeg A en AAAA verslagen toe om het apex domein (`<domain-name>.com`) aan de volgende Snelle IP adressen in kaart te brengen:

   | Apex-domein | ANAME | AAANAME |
   | --------------- | ----------------- | -------- |
...
- [Beveiligde configuratie voor beheerdersreferenties en Basis Admin URL](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/security/security-admin)
...
- [Beveiligde configuratie voor Admin-gebruiker](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/security/security-admin)

- [Beveiligde configuratie voor URL van beheerder](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/site-store/store-urls#use-a-custom-admin-url)
...
- [2-factor verificatie configureren](https://developer.adobe.com/commerce/testing/functional-testing-framework/two-factor-authentication/)
